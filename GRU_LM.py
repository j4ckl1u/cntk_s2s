...
class GRU:
    ...
    def __init__(self, embSize, hiddenSize, inputVocabSize, outputVocabSize):
        ...
        self.Emb = C.layers.Embedding(embSize, init=C.glorot_uniform())
        ...
        self.Parameters = [self.Emb.E, self.Wt, self.Wtb, self.Wr, self.Wrb, self.Wz, self.Wzb, self.W, self.Wb, self.Ur, self.Uz, self.U, self.Ub]

    def createNetwork(self, inputWord, preHidden):
        #self.inputEmb = C.times(inputWord, self.Emb)
        inputEmb = self.Emb(inputWord)
...
class GRULM:
    ...
    def __init__(self, maxLength, vocabSize, embSize, hiddenSize, learningRate = 1.0):
        ...
        self.inputMatrix = C.input_variable(shape=(self.maxLength*self.batchSize, self.vocabSize), is_sparse=True)
    ...
    def createNetworkI(self, length):
        networkHidden = {}
        tce = 0
        input = C.reshape(self.inputMatrix, shape=(self.maxLength, self.batchSize, self.vocabSize))
        for i in range(0, length-1, 1):
            ...
            tce += C.times(C.reshape(ce, shape=(1, self.batchSize)), C.reshape(self.maskMatrix[i], shape=(self.batchSize, 1)))
        return tce
    ...
    def train(self):
        for i in range(0, 100000, 1):
            ...
            trainBatch = self.getTrainBatch()
            (length, count, batchInput, batchMask) = self.buildInput(trainBatch)
            network = self.getNetwork(length)
            batchGrad = network.grad({self.inputMatrix: batchInput,
    ...
    def validate(self):
        ...
        ceAll = 0
        while(valBatch):
            (length,count, batchInput, batchMask) = self.buildInput(valBatch)
            network = self.getNetwork(length)
            countAll += count
            ce = network.eval({self.inputMatrix:batchInput, self.maskMatrix:batchMask})
    ...
    def buildInput(self, sentences):
        sentInput = []
        maxLength = len(max(sentences, key=len))
        maxLength = self.maxLength if maxLength > self.maxLength else maxLength
        for i in range(0, self.maxLength, 1):
            for j in range(0, len(sentences), 1):
                if(i < len(sentences[j])):
                    sentInput.append(sentences[j][i])
                else:
                    sentInput.append(self.getID("</s>"))
        batchInput = C.Value.one_hot(sentInput, self.vocabSize)

        count = 0
        batchMask = np.zeros((self.maxLength, self.batchSize), dtype=np.float32)
        for i in range(0, len(sentences), 1):
            sentence = sentences[i]
            lastIndex = len(sentence) if len(sentence) < self.maxLength else self.maxLength
            for j in range(0, lastIndex, 1):
                batchMask[j, i] = 1
            batchMask[lastIndex - 1, i] = 0
            count += lastIndex-1
        return (maxLength, count, batchInput, batchMask)
